...
int main()
{
    ...
    //set-up cell states
    err = myCA_2D.CA_setup_cell_states(CA_2D, 3);
    if (err==0)
    {
    ...
    err = myCA_2D.CA_init_cond_localized(CA_2D, 1, 1);
    if (err==0)
    {
    ...
    std::vector<int> rule_types = {CONDITIONAL_TRANSITION_RULE_ON_NEIGHBOR, MAJORITY_RULE, MAJORITY_RULE};
