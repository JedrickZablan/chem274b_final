...
int main()
{
    ...
    //set-up boundry
    //2D
    err = myCA_2D.CA_setup_boundrytype(CA_2D, PERIODIC_BOUNDARY , 1);
    if (err==0)
    {
    ...
    //set-up cell states
    err = myCA_2D.CA_setup_cell_states(CA_2D, 4);
    if (err==0)
    {
    ...
    //  Print the CA matrix
    std::cout << "2D CA Matrix after adding state 1 initial condition:" << std::endl;
    ...
    err = myCA_2D.CA_init_cond_rewrite(CA_2D, 2, 0.2);
    if (err==0)
    {
        std::cout<< " 2D CA_init_cond successfull." << std::endl;
    }
    ...
    //  Print the CA matrix
    std::cout << "2D CA Matrix after adding state 2 initial condition:" << std::endl;
    for (int i = 0; i < dim1; ++i)
    {
        for (int j = 0; j < dim2; ++j)
        {
    ...
    err = myCA_2D.CA_init_cond_rewrite(CA_2D, 3, 0.05);
    if (err==0)
    {
        std::cout<< " 2D CA_init_cond successfull." << std::endl;
    }
    ...
    //  Print the CA matrix
    std::cout << "2D CA Matrix after adding state 3 initial condition:" << std::endl;
    for (int i = 0; i < dim1; ++i)
    {
        for (int j = 0; j < dim2; ++j)
        {
    ...
    std::vector<int> rule_types = {STRAIGHT_CONDITIONAL_TRANSITION_RULE, STRAIGHT_CONDITIONAL_TRANSITION_RULE, 
                                    STRAIGHT_CONDITIONAL_TRANSITION_RULE, STRAIGHT_CONDITIONAL_TRANSITION_RULE};
